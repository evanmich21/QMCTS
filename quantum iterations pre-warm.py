...
# === Pre-warm the quantum sampler so setup cost is *not* in your timings ===
# build the largest needed circuit (q=2 for up to 4 moves) and fill its batch
_ = quantum_choice([(None,None)] * 4)
...
def main():
    ...
    for m in preset_mazes:
        ...
        print(f"\n=== Maze {m['name']} ===")
        print(f" Classical avg time: {avg_c:.12f}s, wins {wins_c}/{trials}")
        print(f" Quantum   avg time: {avg_q:.12f}s, wins {wins_q}/{trials}")

if __name__=="__main__":
    main()

#remote address for computer: 1321900494
